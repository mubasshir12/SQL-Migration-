                        ...
                        <motion.div variants={itemVariants} className="col-span-1 border border-gray-200 dark:border-white/10 rounded-3xl p-4 sm:p-5 shadow-sm hover:shadow-md transition-shadow bg-red-50/20 dark:bg-red-900/10 flex flex-col justify-between">
                            ...
                            <div className="flex flex-row gap-2 sm:gap-3">
                                <button 
                                    ...
                                    className="flex-1 flex flex-col items-center justify-center p-3 rounded-2xl bg-gray-50 dark:bg-white/5 border border-transparent hover:bg-gray-100 dark:hover:bg-white/10 transition-all gap-1"
                                >
                                    <div className="flex items-center gap-1.5 sm:gap-2">
                                        <Key className={`w-4 h-4 ${apiKey ? 'text-emerald-500' : 'text-gray-400'}`} />
                                        <span className="text-xs sm:text-sm font-medium text-gray-700 dark:text-white/80 whitespace-nowrap">API Key</span>
                                    </div>
                                    <span className="text-[10px] font-mono text-gray-500 dark:text-white/60">
                                        {apiKey ? `••••${apiKey.slice(-4)}` : 'Setup'}
                                    </span>
                                </button>

                                <button 
                                    onClick={() => setIsResetConfirmOpen(true)}
                                    className="flex-1 flex flex-col items-center justify-center p-3 rounded-2xl bg-red-50/50 dark:bg-red-500/5 border border-transparent hover:bg-red-50 dark:hover:bg-red-500/10 hover:border-red-100 dark:hover:border-red-500/20 transition-all text-red-600 dark:text-red-400 gap-1"
                                >
                                    <div className="flex items-center gap-1.5 sm:gap-2">
                                        <Trash2 className="w-4 h-4" />
                                        <span className="text-xs sm:text-sm font-medium whitespace-nowrap">Reset App</span>
                                    </div>
                                    <span className="text-[10px] font-mono text-transparent">
                                        Hidden
                                    </span>
                                </button>
                            </div>
