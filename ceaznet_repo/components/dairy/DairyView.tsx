...
const DairyView: React.FC<DairyViewProps> = ({ setDairyHeaderState }) => {
    ...
    const handleDownloadPDF = async (options: { rangeType: 'all' | 'specific_month' | 'custom'; month?: number; year?: number; startDate?: string; endDate?: string }) => {
        ...
        filteredEntries.forEach(entry => {
            ...
            const qty = `${entry.quantity} ${selectedItem.unit}`;
            const price = `Rs. ${entry.pricePerUnit}`;
            const total = `Rs. ${entry.totalPrice}`;
            const status = entry.isFullyPaid ? 'Paid' : (entry.paidAmount > 0 ? `Partial (Rs. ${entry.paidAmount})` : 'Unpaid');
        ...
        doc.text(`Default Price: Rs. ${selectedItem.defaultPrice} / ${selectedItem.unit}`, 14, 38);
        doc.text(`Default Quantity: ${selectedItem.defaultQuantity || 1} ${selectedItem.unit}`, 100, 38);
                ...
                if (data.section === 'body') {
                    if (data.row.index === tableRows.length - 1) {
                        ...
                        if (data.column.index >= 0 && data.column.index <= 5) {
                            data.cell.styles.cellPadding = { top: 4, right: 4, bottom: 4, left: 10 };
                        }
            ...
            didDrawCell: function(data) {
                if (data.section === 'body' && data.row.index !== tableRows.length - 1) {
                    ...
                    if (data.column.index === 0 && darkIcons.calendar) {
                        doc.addImage(darkIcons.calendar, 'PNG', data.cell.x + padding, data.cell.y + padding + 0.5, iconSize, iconSize);
                    } else if (data.column.index === 1 && darkIcons.qty) {
                        doc.addImage(darkIcons.qty, 'PNG', data.cell.x + padding, data.cell.y + padding + 0.5, iconSize, iconSize);
                    } else if (data.column.index === 2 && darkIcons.tag) {
                        doc.addImage(darkIcons.tag, 'PNG', data.cell.x + padding, data.cell.y + padding + 0.5, iconSize, iconSize);
                    } else if (data.column.index === 3 && darkIcons.rupee) {
                        doc.addImage(darkIcons.rupee, 'PNG', data.cell.x + padding, data.cell.y + padding + 0.5, iconSize, iconSize);
