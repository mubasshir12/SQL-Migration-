...
const getDocumentConfig = (filename: string) => {
    ...
    if (['xls', 'xlsx', 'csv'].includes(ext)) return { icon: FileSpreadsheet, color: 'text-green-500', bg: 'bg-green-500/10' };
    if (['ppt', 'pptx'].includes(ext)) return { icon: FileText, color: 'text-orange-500', bg: 'bg-orange-500/10' };
                                ...
                                >
                                    {selectedItem.type === 'video' ? (
                                        ...
                                        (() => {
                                            ...
                                            const textExts = ['txt', 'csv', 'json', 'md', 'js', 'ts', 'jsx', 'tsx', 'html', 'css'];
                                            ...
                                            if (ext === 'pdf' || selectedItem.mimeType === 'application/pdf') {
                                                ...
                                                const { icon: DocIcon, color } = getDocumentConfig(selectedItem.filename);
                                                return (
                                                    <div className="flex flex-col items-center justify-center p-8 bg-neutral-900 rounded-2xl shadow-2xl animate-in zoom-in-95 duration-500 w-full max-w-md">
                                                        <DocIcon className={`w-24 h-24 mb-6 ${color}`} />
                                                        <h3 className="text-white text-lg font-medium mb-2 text-center break-all">{selectedItem.filename}</h3>
                                                        <p className="text-neutral-400 text-sm mb-8">{formatBytes(selectedItem.size)}</p>
                                                        <a 
                                                            href={fullScreenUrl} 
                                                            target="_blank" 
                                                            rel="noopener noreferrer"
                                        ...
                                        <div className="flex flex-col items-center justify-center p-8 bg-neutral-900 rounded-2xl shadow-2xl animate-in zoom-in-95 duration-500 w-full max-w-md">
                                            {(() => {
                                                const { icon: DocIcon, color } = getDocumentConfig(selectedItem.filename);
                                                return <DocIcon className={`w-24 h-24 mb-6 ${color}`} />;
                                            })()}
                                            <h3 className="text-white text-lg font-medium mb-2 text-center break-all">{selectedItem.filename}</h3>
                                            <p className="text-neutral-400 text-sm mb-8">{formatBytes(selectedItem.size)}</p>
                                            <a 
